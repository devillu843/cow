...
class DecodeBox():
    def __init__(self, anchors, num_classes, input_shape, anchors_mask=[[6, 7, 8], [3, 4, 5], [0, 1, 2]]) -> None:
    ...
    def decode_box(self, inputs):
        ...
        for i, input in enumerate(inputs):
            ...
            #-----------------------------------------------#
            prediction = input.view(batch_size, len(
                self.anchors_mask[i]), self.bbox_attrs, input_height, input_width).permute(0, 1, 3, 4, 2).contiguous()
    ...
    #-------------------------------------------------------#
            ...
            #   当参数有三个时，第一个参数表示的是复制后的通道数，
            #   第二个参数表示的是复制后的列数，第三个参数表示复制后的行数。
            #-----------------------------------------------#
            ...
            '''
            上面那行代码多么神奇，我一维可以写到一起，结果会报错
            Number of dimensions of repeat dims can not be smaller than number of dimensions of tensor

            wait!!!!!!!!!!!!
            看我发现了什么，实验中结果是一样的，所以我把自己的写法放在下面

            anchor_y=anchor_y.repeat(batch_size, 1，input_height * input_width).view(w.shape)
            '''
    ...
    #-------------------------------------------------------#
            ...
            #   再调整先验框的宽高。
            #   .data返回相同值的tensor，两者同时发生变化
            #   .detach返回相同值的tensor，不同时变化
            #-----------------------------------------------#
            pred_boxes = FloatTensor(prediction[..., :4])
            ...
            output = torch.cat((pred_boxes.view(batch_size, -1, 4) / _scale, conf.view(
                batch_size, -1, 1), pred_cls.view(batch_size, -1, self.num_classes)), -1)
            outputs.append(output.data)
        return outputs

    def yolo_correct_boxes(self, box_xy, box_wh, input_shape, image_shape, letterbox_image):
        #-----------------------------------------------------------------#
        #   所以box_xy到底是什么格式？？  看下面的nms
        #   这个函数是干什么的？？？？？真实框的变换？？？？？？？？？？？？？？？
