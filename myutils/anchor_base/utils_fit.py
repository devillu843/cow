...
from utils.utils import get_lr, preprocess_input
...
def fit_one_epoch(model_train, model, yolo_loss, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda):
    ...
    model:选用的模型
    yolo_loss:自己写的loss函数，YOLOLoss
    loss_history:存储loss
    optimizer:优化器
    epoch:第几个epoch
    epoch_step:num_train // batch_size
    epoch_step_val:num_val // batch_size
    gen:训练数据，经过Dataloader处理
    gen_val:验证数据，经过Dataloader处理
    Epoch:总的epoch数目
    cuda:
    '''
        ...
        for iteration, batch in enumerate(gen):
            ...
            with torch.no_grad():
                if cuda:
                    ...
                    targets = [torch.from_numpy(ann).type(
    ...
    with tqdm(total=epoch_step_val, desc=f'Epoch {epoch + 1}/{Epoch}', position=dict, mininterval=0.3) as pbar:
