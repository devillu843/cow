...
class YoloDataset(Dataset):
    def __init__(self, annotation_lines, input_shape, num_classes, mosaic, train) -> None:
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        #   从指定序列中随机获取指定长度的片断并随机排列，结果以列表的形式返回
        #---------------------------------------------------#
        if self.mosaic:
            if self.rand() < 0.5:
    ...
    def get_random_data_with_Mosaci(self, annotation_line, input_shape, max_boxes=100, hue=.1, sat=1.5, val=1.5):
        #---------------------------------------------------#
        #   我知道这是进行数据增强的，但我没想到一个函数写完了
        #   本以为要一个增强写一个函数，这样主函数会复杂一点
        #   现在这种写法主函数会比较简单
        #   两者后续更改都不麻烦
        #---------------------------------------------------#
        h, w = input_shape
