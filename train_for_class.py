...
# 数据结果存储路径
write_home = './logs/test-41-different/all'
write_name = '/resnet18/'
write_path = write_home + write_name
# 权重文件存储路径
save_home = './weights/test-41-different/all'
save_name = '/resnet18.pt'
save_path = save_home + save_name
# 分类文件
json_class = 'class-test-41.json'
excel_name = 'excel/resnet18_41_all_.xlsx'
...
data_transform = {
    'train': transforms.Compose([transforms.Resize((640, 640)),
    ...
    'val': transforms.Compose([transforms.Resize((640, 640)),
                              transforms.ToTensor(),
                              transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[
                                  0.229, 0.224, 0.225])
                               ]),
    'test': transforms.Compose([transforms.Resize((640, 640)),
                                transforms.ToTensor(),
                                transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[
                                    0.229, 0.224, 0.225])
                                ])
